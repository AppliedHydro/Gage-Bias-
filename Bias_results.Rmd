...
The following figure shows the [Wasserstein Distance](https://www.sciencedirect.com/topics/mathematics/wasserstein-distance) of 13 selected variables.
...
Below are variable distributions showing cumulative probabilities of variable occurance. 
...
The next step is selecting the GRADES river segment that will create the largest reduction in bias for the select variable by creating a 'simulated gage.' Bias calculations are reworked with a fake gage assigned to each successive ungaged segment in each loop. The segment that produces the largest reduction in bias to the select variable is highlighted below in red with adjacent river sections.
...
The same river segment overlayed with satellite imagery
